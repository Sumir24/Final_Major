...
- `.gsd/PLAN.md` — Active execution plan
</context>
...
## 3. Ensure Active Plan Exists

**PowerShell:**
```powershell
if (-not (Test-Path ".gsd/PLAN.md")) {
    Write-Error "PLAN.md not found. Run /plan first."
}
```

**Bash:**
```bash
if [ ! -f ".gsd/PLAN.md" ]; then
    echo "Error: .gsd/PLAN.md not found. Run /plan first." >&2
    exit 1
fi
```
...
Read `.gsd/PLAN.md`. Check if it matches the requested phase.

**If `--gaps-only`:** Filter to only tasks/plans with `gap_closure: true` in frontmatter.

**If phase already marked complete in ROADMAP.md and --redo not set:** (Redo logic is handled in /plan, but here we confirm we are executing the right thing).
...
2. **Verify each task** — Run `<verify>` commands
3. **Commit per task:**
   ```bash
   git add -A
   git commit -m "feat(phase-{N}): {task-name}"
   ```
4. **Append result to ARCHIVE.md** — Log the execution.
